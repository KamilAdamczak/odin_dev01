...
particleEmittersReturnSprite :: proc(particleEmitters : ..ParticleEmitter) -> (sprites : [dynamic]Sprite) {
    
    for particleEmitter in particleEmitters {
        for particle in particleEmitter.particles {
            append(&sprites, particle.sprite)
            // rl.DrawTexturePro(
            //     particle.sprite.texture,
            //     {
            //         f32(particle.sprite.atlas_pos.x) * 16,
...
SpriteDraw :: proc(sprites : ..Sprite) {
    for sprite in sprites {
        rl.DrawTexturePro(
            sprite.texture,
            {
                f32(sprite.atlas_pos.x) * 16,
                f32(sprite.atlas_pos.y) * 16,
                f32(sprite.texture_scale.x),
                f32(sprite.texture_scale.y),
            },
            {0, 0, f32(sprite.texture_scale.x)*1, f32(sprite.texture_scale.y)*1},
            {8, 8}*1,
            0.0,
            rl.WHITE,
        )
